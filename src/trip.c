...
_trip_listen(_trip_router_t *r, trip_socket_t fd, int events)
{
    ...
    while (c)
    {
        ...
        bool wrote = _trip_connection_send(c, buf);
        if (wrote)
        {
            int error = p->send(p->ud, (tripbuf_t *)buf);
        }

        if (_trip_connection_sendmore(c))
        {
            _trip_send_nq(r, c);
        }
        c = _trip_send_dq(r);
...
_trip_segment(_trip_router_t *r, int src, size_t len, unsigned char *buf)
{
    ...
    int end = trip_unpack(len, buf, "eOVV",
                          &prefix.encrypted,
                          &prefix.control,
                          &prefix.id,
                          &prefix.seq);
    // TODO validate control char

    if (end <= 0)
    {
        /* Drop packet. Invalid extraction. */
    ...
    if (_TRIP_CONTROL_OPEN == prefix.control)
    {
        ...
        _trip_open_t open;
        int end2 = trip_unpack(len - end, buf + end, "HbeIQnpIIIE",
                               &open.version,
                               &open.route_len,
                               &open.route,
                               r->decryptbuf,
        ...
        _trip_connection_handle_open(c, &open);
    }
    else
    {
        ...
        if (c)
        {
            _trip_connection_handle_segment(c, src, len - end, buf + end);
        }
        else
        {
